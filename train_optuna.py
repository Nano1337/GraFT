# TODO: put into Chat and lint again. 
...
def objective(trial: Trial, cfgs: dict, fabric: Fabric, rand_seed: int):
    ...
    hyperparams = {
        ...
        "lr":
        trial.suggest_float("lr", 1e-5, 1e-5, log=False),  # default
        "weight_decay":
        trial.suggest_float("weight_decay", 0.001, 0.001, log=False),
        "beta1":
        trial.suggest_float("beta1", 0.80, 0.85),
        "beta2":
        trial.suggest_float("beta2", 0.97, 0.98)
...
def main(cfgs: dict, fabric, trial: Trial = None):
    ...
    train_dataset, val_dataset, train_loader, val_loader = data_utils.get_dataset_and_dataloader(
        cfgs)
