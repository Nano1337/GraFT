...
def weights_init_classifier(m: nn.Module) -> None:
    """Initializes the weights of the classifier using normal initialization.
...
class DEIT_Gradual_Fusion(nn.Module):
    """The DEIT_Gradual_Fusion class extends PyTorch's nn.Module to provide functionality for multiple cls tokens.

    Note:
        Uses the full amount for both ReID and classification.
        Set the number of classification tokens using 'model_num_cls_tokens' in the configuration.
    """

    def __init__(self, cfg: Dict[str, Union[int, str]], fabric: any) -> None:
        """Initializes the DEIT_Gradual_Fusion object.

        Args:
            cfg: The configuration parameters for the model.
        ...
        self.decoder = nn.Linear(
            ...
            cfg['model_decoder_output_class_num'],
            bias=False)
        ...
        self.decoder = nn.Linear(
            (self.cfg.model_num_fusion_tokens +
             self.cfg.model_num_cls_tokens * len(self.cfg.model_modalities)) *
            hidden_size,
            cfg.model_decoder_output_class_num,
            bias=False)
    ...
    def forward(
        ...
        """Feeds input through the model.

        Args:
            inputs: Dictionary of modality-specific tensors.
            train_mode: Boolean flag indicating whether to run in training mode.

        Returns:
            Depending on the training mode, either a dictionary of embeddings or a tuple of class outputs & embeddings.
        """
        ...
        for modality in self.cfg['model_modalities']:
            ...
            z_anchors[modality] = z_anchors[
                modality].last_hidden_state.permute(1, 0, 2)

            cls_anchor[modality] = self.cls_anchor[modality].repeat(
                1, z_anchors[modality].shape[1], 1)
            if self.cfg['model_fusion_combos'][0] == "f":
                # concat in order of cls, fusion, data
                z_anchors[modality] = torch.cat(
                    (cls_anchor[modality],
                     self.fusion_tokens.repeat(1, z_anchors[modality].shape[1],
                                               1), z_anchors[modality]),
                    dim=0)
            ...
            fusion_anchor = z_anchors_joint[
                modality][:, self.cfg['model_num_cls_tokens']:self.
                          cfg['model_num_cls_tokens'] +
                          self.cfg['model_num_fusion_tokens'], :]
            ...
            for modality in self.cfg['model_modalities']:
                ...
                if self.cfg['model_fusion_combos'][1] == "f":
                    z_pos[modality] = torch.cat(
                        (cls_pos[modality],
                         self.fusion_tokens.repeat(1, z_pos[modality].shape[1],
                                                   1), z_pos[modality]),
                        dim=0)
            ...
            for modality in self.cfg['model_modalities']:
                ...
                if self.cfg['model_fusion_combos'][2] == "f":
                    z_neg[modality] = torch.cat(
                        (cls_neg[modality],
                         self.fusion_tokens.repeat(1, z_neg[modality].shape[1],
                                                   1), z_neg[modality]),
                        dim=0)
